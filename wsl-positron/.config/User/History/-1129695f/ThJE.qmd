...
```{r}
design |>
  group_by(definition_one, riagendr) |>
  summarise(
  ...
  group_by(definition_one) |>
  mutate(
    row_pct = prop / sum(prop) * 100,
    row_pct_low = prop_low / sum(prop) * 100,  # Approximate
    row_pct_upp = prop_upp / sum(prop) * 100
  )

svyby(
  formula = ~definition_one,
  by = ~riagendr,
  design = design,
  FUN = svymean,
  vartype = "ci"
...
svyby(
  ...
  FUN = svymean,
  vartype = "ci",
  method = "logit"  # Use logit transformation
)
```