...
design_int <- create_design(defined, min(defined$year), max(defined$year), 'int')
design_mec <- create_design(defined, min(defined$year), max(defined$year), 'mec')
```
...
left_join(def1, def2, by = 'group') |> 
  ...
  tab_caption(caption = md(
    '**TABLE 2.5: Prevalence of Type 1 Diabetes, by Age, Sex, and Race/Ethnicity, National Health and Nutrition Examination Surveys, U.S., 1999–2010**'
  ))
```