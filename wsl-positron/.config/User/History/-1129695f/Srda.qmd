...
design |> 
  ...
  select(definition_one, `Pct (95% CI)` = result)
...
design |> 
  ...
  select(definition_two, `Pct (95% CI)` = result)
```

```{r}
#| label: definition vs age group
design |> 
  group_by(definition_one, age_group) |> 
  summarize(
    prop = survey_mean(vartype = "ci"),
    total = survey_total(),
  ) |> 
  ungroup() |> 
  mutate(
    group = glue::glue('{definition_one} {age_group}'),
    result = glue::glue(glue_content)
  ) |> 
  select(group, `Pct (95% CI)` = result)

design |> 
  group_by(definition_two, age_group) |> 
  summarize(
    prop = survey_mean(vartype = "ci"),
    total = survey_total()
  ) |> 
  mutate(
    group = glue::glue('{definition_two} {age_group}'),
    result = glue::glue(glue_content)
  ) |> 
  select(group, `Pct (95% CI)` = result)
```