...
<!-- There is a file that loads the treatment failure information. We have saved that file to speed up the run time of the code but you can see the algorithm and details in the ***create_outcome.R*** file:
...
as_flextable(variables_table, hide_grouplabel = TRUE) |>
  ...
  # add_footer_lines("Table 1:  Features used to predict treatment failure.") |>
...
table_one_gt_tbl <-
  ...
  # tab_footnote(
  #   footnote = 'Table 1: Features used to predict treatment failure.'
  # ) |> 
  tab_row_group(
...
final_analysis |>
  table1::table1(~. | trial, data = _, footnote = "Table 2:  Descriptive statistics.")
```
...
While it is possible to search every combination of *k* and weights by creating a grid, typically a large number of random combinations are tried to cover the space of possibilities. A "space-filling" design can be used to make sure the randomly chosen values are not too clumped together. You will see examples of grids in the code below for methods that are not tuning many hyperparameters (e.g., LASSO) as well as a space-filling designs for methods that need to tune many parameters (e.g., neural networks). The state-of-the-art in ML focuses on how to choose options and then check other possible values in the neighborhood that look promising using *Bayesian optimization* methods. Watch for those Bayesian methods in future work.
...
Prior to modeling, all data were preprocessed using the same steps which were programmed using the R `recipies` package (version `r packageVersion("recipes")`). That code, which is shown in [Workflow]() section above, is repeated here.
...
We chose to include this complex model because we wished to evaluate its performance against the more conventional methods used in our area of research, but we unable to achieve greater performance metrics with a neural network. It is important to and reiterate the importance of careful model tuning and interpretation when equipped with other comparative models.
:::
::::
...
Saving the `finalize_workflow(knn_workflow, knn_details)` object is how we accept and choose the best performing model. Our `knn_final` object is the model that will we will use to obtain performance metrics on the training and testing data.
:::
:::::
...
The `fit()` function requires that pass our training data (`a_train`) to the best performing model (`knn_final`). The `knn_final_fit` object holds the parameter estimates for our model on the **training** data.
...
Notice that we further clean our `augment`ed training data and create a column `estimate` that is the predicted probability of treatment failure. If we were to instead use `.pred_1` our ROC AUC will not be correct in demonstrating the model's ability to predict treatment failure. *Knowing which level of outcome is important in your own predition modeling.*
...
The `roc_auc()` function receives the training data with new preditions and we specify that the observed outcome (`truth`) is in the `did_relapse` column, but assess that against the prediction of `estimate` (the predicted probability of failure).
