...
######################################################################################
time <- glue::glue('{min(the_data$year)}-{max(the_data$year)}')
...
full_count <- nrow(study_cohorts$data$.full) |> scales::comma()
initially_excluded <- nrow(study_cohorts$data$initially_excluded) |> scales::comma()
controls <- nrow(study_cohorts$data$controls) |> scales::comma()
...
study_cohorts |> 
  ...
  consort_box_add(
    'full', 0, 160, 
    glue::glue('<strong>NHANES<br>({time})</strong><br>(n = {full_count})')
  ) |> 
  consort_box_add(
  ...
  consort_box_add(
    ...
    glue::glue('Fasting blood<br>glucose >= 126<br>(n = {fpg_count})')
  ) |>
