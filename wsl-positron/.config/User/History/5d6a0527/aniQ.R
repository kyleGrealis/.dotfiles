...
######################################################################################
time <- glue('{min(the_data$year)}-{max(the_data$year)}')
...
study_cohorts <- the_data |> 
  ...
  cohort_define(
    ...
    # Control group -- didn't meet inclusion, but no with undiagnosed diabetes
    controls = initially_excluded |> anti_join(undiagnosed_type2, by = 'seqn')
  ) |> 
  cohort_label(
    .full = glue('<strong>NHANES<br>({time})</strong><br>'),
    # to assess for controls vs undiagnosed
    initially_excluded = 'Initially excluded',
    undiagnosed_type2_both = 'Undiagnosed Type 2 (FPG & HbA1c)',

    step_1_insulin = 'Invalid response for insulin',
    step_1_pills = 'Invalid response for diabetic pills',
    step_1_duration = 'Invalid insulin duration',
    
    step_1_continue = 'Yes<br>',
    type_one = 'Type 1 (step 2)',
    step_2 = 'No<br>',
    type_two_step_3 = 'No<br>',
    step_3 = 'Yes<br>',
    type_two_step_4 = 'No<br>',
    step_4 = 'Yes<br>',
    type_two_step_5 = 'No<br>',
    not_classified = 'Has diabetes diagnosis<br>but not classified<br>'
  )
summary(study_cohorts) |> print(n=50)

full_count <- nrow(study_cohorts$data$.full) |> comma()
initially_excluded <- nrow(study_cohorts$data$initially_excluded) |> comma()
controls <- nrow(study_cohorts$data$controls) |> comma()
fpg_count <- nrow(study_cohorts$data$undiagnosed_type2_fpg) |> comma()
hba1c_count <- nrow(study_cohorts$data$undiagnosed_type2_hba1c) |> comma()
undx_type2 <- nrow(study_cohorts$data$undiagnosed_type2) |> comma()
met_inclusion <- nrow(study_cohorts$data$met_inclusion) |> comma()
step_1_removed <- nrow(study_cohorts$data$step_1_removed) |> comma()
total_t2dm <- 
  (nrow(study_cohorts$data$type_two_step_3) +
    nrow(study_cohorts$data$type_two_step_4) +
    nrow(study_cohorts$data$type_two_step_5)) |> 
  comma()
type_1 <- nrow(study_cohorts$data$type_one) |> comma()

consort_diagram <- study_cohorts |> 
  # Initial entry
  consort_box_add(
    'full', 0, 160, 
    glue('<strong>NHANES<br>({time})</strong><br>(n = {full_count})')
  ) |> 
  consort_box_add(
    'exclusions_1', 3, 160, glue(
    '<strong>Initially excluded</strong> (n = {initially_excluded})<br>
    • Without complete education information<br>
    • Without prior diagnosis of diabetes<br>
    • Without valid reported age at diabetes diagnosis<br>
    • Without valid duration of diabetes
    ')
  ) |> 
  ...
  consort_box_add('fudge_it', 10, 140, '') |> 
  consort_box_add(
    'diabetes_not_yes', 10, 140, 
    'Assess for<br>undiagnosed<br>diabetes'
  ) |> 
  # Point to Participants without diagnosed diabetes
  consort_line_add(
    # start_x = 0, start_y = 135, end_x = 16, end_y = 135
    start_x = 10, start_y = 160, end_x = 10, end_y = 135
  ) |> 
  # Point from Assess for undiagnosed to Controls
  consort_line_add(
    start_x = 10, start_y = 140, end_x = 16, end_y = 140
  ) |> 
  # Fasting blood glucose >= 126
  consort_box_add(
    'fpg', 5, 125, 
    glue('Fasting blood<br>glucose >= 126<br>(n = {fpg_count})')
  ) |>
  ...
  consort_box_add(
    'hb', 15, 125, 
    glue('HbA1c<br>>=6.5<br>(n = {hba1c_count})')
  ) |>
  ...
  consort_box_add(
    'step_0c', 0, 106, 
    glue('Participants with<br>diagnosed diabetes<br>(n = {met_inclusion})')
  ) |> 
  # Right side box
  ...
  consort_box_add(
    'step_one_exclusions', 3, 77, glue(
    '<strong>Removed during Step 1</strong> (n = {step_1_removed})<br>
  ...
  ggplot2::annotate(
    ...
    x = 16, y = 140,
    label = glue(
      '<strong>Controls</strong><br>(n = {controls})<br>Total not diagnosed<br>with diabetes'
    ),
    fill = "#cf9bfcff",
    color = "black"
  ) +
  ...
  ggplot2::annotate(
    geom = "richtext",
    x = 10, y = 106,
    label = glue('<strong>Undiagnosed<br>Type 2 DM</strong><br>(n = {undx_type2})'),
    fill = "#fc9be2ff",
    color = "black"
  ) +
  ...
  ggplot2::annotate(
    geom = "richtext",
    x = 10, y = 58,
    label = glue('<strong>Type 1 DM</strong><br>(n = {type_1})'),
    fill = "#9bfcd2ff",
    color = "black"
  ) +
  ...
  ggplot2::annotate(
    geom = "richtext",
    x = 10, y = 18, 
    label = glue('<strong>Type 2 DM</strong><br>(n = {total_t2dm})'),
    fill = "#9bc0fc",
    color = "black"
  ) +
...
consort_diagram
save(consort_diagram, file = 'data/consort_diagram_09252025.RData')
# image saved into misc/img/consort_09252025.png
