...
#' @param start_yr Numeric. Start year of analysis.
#' @param end_yr Numeric. End year of analysis.
...
#' category is to be used. Choose either of \code{"interview"}, \code{"mec"}, or 
#' \code{"fasting"}.
#' 
#' @examples
#' \dontrun{
...
create_design <- function(
  ...
  # Count the number of dataset cycles
  ...
  message(sprintf(
    'Do not perform any analysis using variables that have a greater selection probability than \"%s\".', wt_type
  ))
  return(design)

}

# create_design(the_data, '1999', 2001, wt_type = 'int')  # error
# create_design(the_data, 1999, '2001', wt_type = 'int')  # error
# create_design(the_data, 1999, 2001, wt_type = 'int')    # good
# create_design(the_data, 1999, 2001, wt_type = 'mec')    # good
# create_design(the_data, 1999, 2001, wt_type = 'f')      # good
# create_design(the_data, 1999, 2001, wt_type = 'interiew')  # error (spelling)
# create_design(the_data, 1999, 2001, wt_type = c('m'))   # good


# x <- create_design(the_data, 1999, 2001, wt_type = 'mec')
