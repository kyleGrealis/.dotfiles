...
# Kyle: source the file that will rework site masked
# source('code_with_notes.R')  # this is handled in do_everything.R
...
# all_drugs -----
# These data are a pain because they are so granualar. I will use `tlfb instead`.
...
analysis <-
  ...
  mutate(detox_days = if_else(is.na(detox_days), runif(nrow(analysis), min = -0.001, max = 0.001), detox_days))
# mutate(detox_days = if_else(is.na(detox_days), rpois(nrow(analysis), lambda = 5), detox_days))
rm(detox)
# nrow(analysis)  # 2492
...
almost_analysis <-
  ...
  select(-x) |>
  # drop participants that have no self-reported pre-randomization drug use info
...
analysis2 <- 
  ...
  select(
    ...
    -c(
      ...
      speedball_inject_days, opioid_inject_days, speed_inject_days, tlfb_days_of_use_n
    )
  )
# nrow(analysis2)  # 2478
...
# nrow(alternative_analysis)  # 2478
