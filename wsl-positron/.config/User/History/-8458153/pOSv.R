# Replicating the original consort diagram from Mosslemi 2020.
# This uses the years 1999 through 2016.
...
nhanes_consort <- function(func_data, time) {
  ...
  cohort_define(
    ...
    step_2 = step_1_continue |> 
      ...
      # filter(diq050 == 'No', diq070 == 'No'),  # as described in the paper
      filter(diq070 == 'No'),
    ...
    step_3 = step_2_continue |> 
      # filter(diq050 == 'No'),  # as described in the paper
      filter(diq050 != 'Yes'),
...
mosslemi_consort_replicated <- the_data |> 
  filter(between(year, 1999, 2016)) |>
  nhanes_consort('1999-2016')
mosslemi_consort_replicated
# No we don't...
# 
# I had to reduce the full NHANES data to ages >= 16 years old. That allowed us to 
# nearly match the total N entering at Step 1. We are within 6 participants difference.
# Our algorithm is now based loosely on the original manuscript wording. For example,
# if the criteria was "A and B", replicating A & B would yield different results than
# to, say, restrict to only condition A *or* condition B.
# IMPORTANT: See the notes for Steps 2 & 3 above and notice the comments "as described
# # in the paper" for clear examples where our method differs to visualize what was
# needed to obtain results that were closer than adhering to strict wording.

# Our prevalence of each category is not vastly different now:
# Type     Mosslemi  (N=5457)          Us  (N=5425)
# T2DM         4732  (86.71%)        4727  (87.13%)
# T1DM          280   (5.13%)         274   (5.05%)
# PosT2         445   (8.16%)         424   (7.82%)

# original study cohort 1999-2016
save(mosslemi_consort_replicated, file = 'data/mosslemi_consort_replicated.RData')
