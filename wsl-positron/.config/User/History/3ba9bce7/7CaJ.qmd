...
cohort <- the_data |> 
  ...
  # October 13: create age groups
  mutate(
    age_groups = case_when(
      between(ridageyr, 30, 39) ~ '30-39',
      between(ridageyr, 40, 49) ~ '40-49',
      between(ridageyr, 50, 59) ~ '50-59',
      .default = 'check'
    )
  )
...
# 2. Weighted: age groups, sex, ethnicity
other <- design_int |> 
  filter(use_category != 'Missing') |> 
  mutate(use_category = fct_drop(use_category)) |> 
  tbl_svysummary(
    by = use_category,
    include = c(age_groups, riagendr, ridreth1),
    statistic = list(all_continuous() ~ '{mean} ({sd})')
  ) |> 
  add_overall() |> 
  add_auto_labels() |> 
...
tbl_stack(
  ...
  modify_caption(
    text_interpret = 'md',
    caption = glue(
      '<strong>Table 1:</strong> Socio-demographic characteristics by cannabis use status
        \n<i>NHANES {years}</i>'
    )
  )
```
...
cohort2 <- 
  ...
  mutate(
    use_category = factor(
      use_category, 
      levels = c('Never used', 'Former user', 'Current light user', 'Current heavy user'),
      ordered = TRUE
    )
  )
design_mec <- create_design(cohort2, 2009, 2018, 'mec')

svyglm(kdm_bioage ~ factor(use_category, ordered = FALSE) + riagendr, design = design_mec) |> 
  tidy()
...
svyglm(kdm_advance ~ factor(use_category, ordered = FALSE) + riagendr + factor(education, ordered = FALSE), design = design_mec) |> 
  tidy()
svyglm(kdm_bioage ~ fev1_1000, design = design_mec) |> tidy()
