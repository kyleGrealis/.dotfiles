...
cohort |> 
  tbl_summary(
    by = riagendr,
    include = c(
      ridageyr, ridreth1, dmdeduc2, income,
    ),
...
cohort |> 
  ...
  tbl_summary(
    by = use_category,
    include = c(
      ridageyr, ridreth1, dmdeduc2, education, income,
    ),
    type = c(ridageyr ~ 'continuous2'),
    statistic = list(
      all_continuous2() ~ c('{mean} ({sd})', '{min}--{max}')
...
cohort |> 
  tbl_summary(
    by = riagendr,
    include = c(
      lbxwbcsi, lbdldl, lbxtc, lbdhdd, bmxbmi, bmxwaist, mean_sbp, mean_dbp, waist_height_ratio, lbxglu, lbdglusi, lbdhi
    ),
    statistic = list(all_continuous() ~ '{mean} ({sd})'),
    missing = 'no'
  ) |> 
...
cohort |> 
  tbl_summary(
    ...
    include = c(
      ...
      mcq220, smq020, urdact, lbdscrsi, lbxscr, egfr_2021
    ),
    statistic = list(all_continuous() ~ '{mean} ({sd})')
    # missing = 'no'
  ) |> 
