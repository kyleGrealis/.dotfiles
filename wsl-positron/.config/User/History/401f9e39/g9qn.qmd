...
#' Vectorized data pull using NHANES data structure
#' @param nhanes_table Character. Quoted string like 'DEMO' or 'LAB'. Must correspond 
#'   to real table prefix. Do not use the '_<letter>', only the base of the table name.
#' @param selected_variables Concatenated list of variable names. Case-insensitive.
#' 
#' @returns Complete dataset from 1999 through 2021. Skips 2019-2020 data. See full
#'   NHANES documentation for reasons (hint: COVID-19). The first year of the cycle is
#'   used to represent that cycle (i.e., year == 2021 is for 2021-2022).
#' @note The returned dataset does not include "<nhanes_table>_K" as the K dataset
#'   corresponds to 2019-2020 data. See full NHANES docs not included here.
...
pull_nhanes <- function(nhanes_table, selected_variables) {
  ...
  mapped_dfr <- start_dfr |> 
    pmap_dfr(function(code, year) {
      message(sprintf('Processing NHANES data for year: %s', year))
      data <- nhanes(code)
...
diab_vars <- c('SEQN', 'DIQ010', 'DID040', 'DIQ160', 'DIQ050', 'DID060', 'DIQ070')
...
# load('data/the_data.rda') #-- this one will be overwwritten so do NOT redo this!!!!!
